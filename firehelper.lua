...
local fires_list = {
                    ...
                    {2244.5944, -30.4941, 26.5403, 1, 'Церковь в паломино крик'},
                    {-193.0916, 1023.3978, 19.7953, 1, 'Пожар на складе в Форт Карсоне'},
                    {1642.4234, 2180.4091, 11.0258, 1, 'Пожар в мастерской Лас Вентураса'},
...
function main()
    ...
    if cnt_client['cnt'] == '0' then
        ...
        assert(conn:execute("INSERT INTO clients (nick, tlg_id, firehelper, lastlogin, ver) VALUES ('"..who_nick.."', '0', '1', '"..lastlogin.."', '"..thisScript().version.."')"))
...
function sampev.onServerMessage(color, text)
    ...
    if text:find('Хочу в отпуск') then
            lua_thread.create(function()
                wait(1000)
                nick_give = string.match(text,"%a+_%a+")
                if nick_give == 'Irin_Crown' and who_nick ~= 'Irin_Crown' then
